  ...
  async fetch(request){
    ...
    if(url.pathname.startsWith('/api/admin/')){
      ...
      if(request.method==='POST'&&url.pathname==='/api/admin/action'){
        const a=b.action;if(a==='start')await this.startGame();else if(a==='pause'&&this.state.phaseEndAt&&!this.state.paused){this.state.pausedRemaining=Math.max(0,this.state.phaseEndAt-Date.now());this.state.phaseEndAt=null;this.state.paused=true;await this.persistState();this.broadcast();await this.scheduleNextAlarm();}else if(a==='resume'&&this.state.paused){this.state.phaseEndAt=Date.now()+(this.state.pausedRemaining||0);this.state.paused=false;this.state.pausedRemaining=null;await this.persistState();this.broadcast();await this.scheduleNextAlarm();}else if(a==='reveal')await this.revealQuestion();else if(a==='next'){if(this.state.phase==='reveal')await this.rankingScreen();else if(this.state.phase==='ranking')await this.loadingScreen();else if(this.state.phase==='loading')await this.nextQuestion();else if(this.state.phase==='suspense')await this.finalScreen();else if(this.state.phase==='question')await this.revealQuestion();}else if(a==='final')await this.suspenseScreen();else if(a==='showfinal'&&this.state.phase==='suspense')await this.finalScreen();else if(a==='finish'){const oldSettings={...this.state.settings},base=this.state.baseUrl;this.state=freshState();this.state.settings=oldSettings;this.state.baseUrl=base;await this.persistState();this.broadcast();await this.scheduleNextAlarm();}else if(a==='reset'){const keep=this.activePlayers().filter(p=>!p.isBot).map(p=>({...p,score:0,answer:null}));const oldSettings={...this.state.settings},base=this.state.baseUrl;this.state=freshState();this.state.settings=oldSettings;this.state.baseUrl=base;this.state.players=keep;await this.persistState();this.broadcast();await this.scheduleNextAlarm();}return json({ok:true});
      }
