...
function lobby(){
  screen.innerHTML=`<div class="screen-content lobby enter-anim"><section class="card lobby-hero"><div><span class="badge">QUIZ CORPORATIVO</span><h1 class="host-title">Quiz <span class="accent">SIPAT 2026</span><br>Automazoom</h1><p class="host-sub">Conhecimento que conecta. Segurança que se pratica.</p></div><div class="join-box qr-join-box"><div class="qr-lobby-copy"><div class="muted">ENTRE PELO CELULAR</div><div class="join-url-box"><div class="join-url-label">Link de acesso</div><div class="join-url">${esc(PLAYER_JOIN_URL)}</div></div><div class="muted room-label">Sala única</div><div class="join-note">Aponte a câmera do celular para o QR Code ou acesse o link ao lado.</div></div><div class="qr-lobby"><div class="qr-frame"><img src="/assets/join_qr.png" alt="QR Code para entrar no Quiz SIPAT 2026"></div><strong>Escaneie para entrar</strong></div></div><div class="safety-note"><img src="/assets/cipa.png" class="cipa-logo"><div><strong>Segurança é compromisso de todos.</strong><div class="muted">Escolha seu avatar e aguarde o início.</div></div></div></section><section class="card players-card"><div class="players-head"><div><h2>Jogadores conectados</h2><div class="muted">Até 31 participantes.</div></div><span class="badge"><span class="dot"></span><span id="liveCount">${S.players.length}/${S.maxPlayers}</span></span></div><div id="lobbyPlayerGrid" class="player-grid" data-signature="${S.players.map(p=>`${p.id}:${p.avatar}:${p.name}:${p.isBot?1:0}`).join('|')}">${lobbyPlayerTiles()}</div></section></div>`;
}
